...
public class KasuP3registerCL{
	static int c =0;
        ...
        try {
            ...
            KasuP3Subscriber.u = new KasuP3User(name, password, type);
            try {
                
                KasuP3Subscriber.ss = new ServerSocket(0);
                KasuP3Subscriber.sendConnectionDetails(KasuP3Subscriber.ss.getLocalPort(), KasuP3Subscriber.ss.getInetAddress().getHostName());
                KasuP3Subscriber.socket = KasuP3Subscriber.ss.accept();
            } 
            ...
            if(type.equals("publisher")) {
            	...
            	KasuP3Subscriber.sendTopicType(topic);
            	//Server.addTopic(topic,c);
            }
            else if(type.equals("subscriber")) {
            }
            
            KasuP3Subscriber.msgUi = new KasuP3messageCL();
            if(type.equals("subscriber")) {
            	...
            	if(!topic.equals("DONE")) {
            		KasuP3Subscriber.sendSubTopics(topic);
        		topic = sc.next();
        		if(!topic.equals("DONE")) {
            		KasuP3Subscriber.sendSubTopics1(topic);
        		topic = sc.next();
        		
        		if(!topic.equals("DONE")) {
            		KasuP3Subscriber.sendSubTopics2(topic);
        		topic = sc.next();
        	
        		if(!topic.equals("DONE")) {
            		KasuP3Subscriber.sendSubTopics3(topic);
        		topic = sc.next();
        	
        		if(!topic.equals("DONE")) {
            		KasuP3Subscriber.sendSubTopics4(topic);
        		topic = sc.next();
        		}
        		}
        		}
